            ...
            <div key={i} style={{background:isToday?"#EEF0FF":isPast?"#FAFAFA":"#fff",border:`1px solid ${isToday?"#5B6EB5":"#E8E8EC"}`,borderRadius:10,minHeight:120,overflow:"hidden"}}>
              ...
              <div style={{padding:"6px 5px"}}>
                {dayEvts.map(e=>{
                  ...
                  const pri=PRIORITY_CONFIG[e.priority]||PRIORITY_CONFIG.medium;
                  return(<div key={e.id} onClick={()=>onEdit(e)} title={`${e.title} · ${e.time||""} · ${e.kid}`} style={{background:cat.color,color:"#fff",borderRadius:5,padding:"3px 6px",marginBottom:4,fontSize:10,fontWeight:700,cursor:"pointer",lineHeight:1.3,borderLeft:`3px solid ${pri.color}`}}>
                    <div style={{whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{e.time?`${e.time} `:""}{e.title}</div>
                    <div style={{fontSize:9,opacity:0.85}}>👤 {e.kid}</div>
                  </div>);
                })}
      ...
      {(()=>{
        ...
        return wEvts.length>0?(
          ...
          <div style={{textAlign:"center",padding:"30px 0",color:"#ccc"}}>
            ...
            No events this week. Click + on any day to add one.
          </div>
        );
      })()}
    </div>
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    const unsub=onSnapshot(q,
      (snap)=>{setEvents(snap.docs.map(d=>({id:d.id,...d.data()})));setLoading(false);setDbError(null);},
  ...
  const DBBanner=()=>dbError?(<div style={{background:"#FEF0EC",border:"1px solid #E05C3A",borderRadius:10,padding:"12px 16px",marginBottom:16,fontSize:13,color:"#C0351A"}}>⚠️ <strong>Firebase not connected.</strong> Check your secrets in GitHub. <a href="https://console.firebase.google.com" target="_blank" rel="noopener noreferrer" style={{color:"#C0351A"}}>Firebase Console →</a><br/><small style={{opacity:0.7}}>{dbError}</small></div>):null;
  ...
  return(
    <div style={{fontFamily:"'Inter','Segoe UI',sans-serif",background:"#F4F4F8",minHeight:"100vh",paddingBottom:40}}>
      <div style={{background:"linear-gradient(135deg,#1A1A2E 0%,#2D2D5E 100%)",padding:"22px 20px 0",color:"#fff"}}>
        ...
        <div style={{marginTop:12,background:"rgba(255,255,255,0.08)",borderRadius:8,padding:"7px 12px",fontSize:11,color:"#B0B0D0",display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
          🔥 Firebase Firestore ·&nbsp;
          <a href={`https://calendar.google.com/calendar/r?authuser=${encodeURIComponent(CALENDAR_EMAIL)}`} target="_blank" rel="noopener noreferrer" style={{color:"#7EC8E3",fontWeight:700}}>{CALENDAR_EMAIL}</a>
          &nbsp;· {loading?"Loading…":`${events.length} events`}
          {!dbError&&!loading&&<span style={{color:"#7EC8E3"}}>✓ Live</span>}
        </div>
      ...
      <div style={{padding:"18px 14px",maxWidth:900,margin:"0 auto"}}>
        <DBBanner/>
        {loading?(
          <div style={{textAlign:"center",padding:60,color:"#aaa"}}><div style={{fontSize:36,marginBottom:12}}>⏳</div>Loading events…</div>
        ):(
          <>
            {tab==="dashboard"&&(
              <>
                <div style={{display:"flex",gap:8,marginBottom:18,flexWrap:"wrap"}}>
                  ...
                  {statCard("Upcoming",upcoming.length,"📌","#2E8B57")}
                </div>
                ...
                <div style={sectionStyle}>
                  <div style={sectionHeader}><span>📅 This Week</span><button onClick={()=>setTab("week")} style={{fontSize:11,color:"#5B6EB5",background:"none",border:"none",cursor:"pointer",fontWeight:700}}>Week view →</button></div>
                ...
                {highPri.length>0&&(<div style={{...sectionStyle,borderLeft:"4px solid #E05C3A"}}><div style={sectionHeader}><span>🔥 High Priority — Don't Miss</span></div>{highPri.map(e=><EventCard key={e.id} event={e} onEdit={setModal} onDelete={deleteEvent}/>)}</div>)}
              </>
            )}
            ...
            {tab==="priority"&&Object.entries(PRIORITY_CONFIG).map(([pk,pv])=>{
              const group=upcoming.filter(e=>e.priority===pk);
